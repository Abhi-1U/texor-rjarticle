...
The R Journal is the primary open-access outlet for publications produced by the R community. It was started in 2009, evolving from a newsletter that ran from 2001, to become a more formal publication that encourages and provides credit for the documentation of statistical computing research. 
...
- Edit the lines where figures are included. <!--Why? What will need changing?-->
- Edit the tables. <!--If the simple conversion is insufficient?-->
- Include your R code, to dynamically do the computing described in your article. 

XXX - Should we include various options/params for the function here ?
...
- extracting `bbl` chunks from a `.tex` file;
...
- aggregating references from embedded `bbl` with references from supplementary `.bbl` and `.bib` files.
