...
```{r workflow, fig.cap="Workflow of the document conversion conducted by texor."}
...
**DC: Don't really understand this example, why would html figure definition also create pdf?**
...
The numbering of equations is a bit trickier. LaTeX automatically numbers equations, unless specifically instructed not to. Equation numbering in R Markdown requires specific labelling using `(\#eq:xx)` as described in @bookdown. The \CRANpkg{texor} helps by adding the labelling using a Lua filter to convert the existing `\label{..}` to `(\#eq:xx)`.
