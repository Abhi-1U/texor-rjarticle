...
which poses an issue during conversions. RMarkdown is quite a bit restrictive by 
...
supported or defined due to lower-level tex limitations in pandoc. Hence the code 
environments used to be eliminated during the conversion process, including a Lua
filter to find these elements and redefine them as code environments would also 
fail because pandoc would have already tried to process the code inside an unknown 
code environment, leading to missing code elements lost in the conversion.	
